...
app.post('/login',async c=>{
 const {username,password}=await c.req.json();if(!validText(username,128)||!validText(password,256))return jsonErr(c,'Invalid username or password',401);const u=await c.env.DB.prepare('SELECT * FROM users WHERE LOWER(username)=LOWER(?) AND active=1').bind(String(username).trim()).first<LoginUser>();if(!u||digest(String(password))!==u.password_hash)return jsonErr(c,'Invalid username or password',401)
