...
async function auth(c:any):Promise<Auth|null>{
...
function hasPerm(a:Auth|null,p:string){return !!a&&(a.kind==='ADMIN'||!!a.user&&JSON.parse(a.user.permissions_json||'[]').includes(p))}
...
async function recomputeBooking(db:D1Database,ref:string){
 ...
 const countRow=await db.prepare(`SELECT COUNT(*) AS n FROM bookings WHERE status!='DONE' AND DATE(created_at)=? AND ((created_by_type='SALESMAN' AND created_by_user_id=?) OR (created_by_type IN ('DOCTOR','REP') AND assigned_to_user_id=?))`).bind(day,ownerId,ownerId).first<any>()
 ...
 if(b.created_by_type==='SALESMAN'&&b.assigned_to_user_id){
   visitFee=owner.user_type==='DOCTOR'?0:0
   const assigned=await userById(db,Number(b.assigned_to_user_id));
   visitFee=assigned?.user_type==='DOCTOR'?Number(s.doctor_visit_fee):assigned?.user_type==='REP'?Number(s.rep_visit_fee):0
 }
 await db.prepare(`UPDATE bookings SET commission_owner_user_id=?,commission_owner_type=?,commission_base_rate=?,commission_bonus_rate=?,commission_threshold=?,commission_base_amount=?,commission_bonus_amount=?,commission_amount=?,daily_patient_count=?,visit_fee_rate=?,visit_fee_amount=?,updated_at=CURRENT_TIMESTAMP WHERE reference=?`).bind(ownerId,ownerType,baseRate,bonusRate,threshold,base,bonus,base+bonus,count,visitFee,visitFee,ref).run()
}
async function freezeFinancials(db:D1Database,ref:string){
 const b=await bookingByRef(db,ref);if(!b)return
 await recomputeBooking(db,ref)
 const x=await bookingByRef(db,ref);if(!x)return
 await db.prepare(`UPDATE bookings SET commission_base_rate=?,commission_bonus_rate=?,commission_threshold=?,commission_base_amount=?,commission_bonus_amount=?,commission_amount=?,visit_fee_rate=?,visit_fee_amount=?,updated_at=CURRENT_TIMESTAMP WHERE reference=?`).bind(x.commission_base_rate,x.commission_bonus_rate,x.commission_threshold,x.commission_base_amount,x.commission_bonus_amount,x.commission_amount,x.visit_fee_rate,x.visit_fee_amount,ref).run()
}
...
app.post('/bookings',async c=>{
 const a=await auth(c);const b=await c.req.json();if(!b.patient_name||!b.phone||!Array.isArray(b.tests)||!b.tests.length)return jsonErr(c,'Invalid booking')
...
app.post('/login',async c=>{
 const {username,password}=await c.req.json();const u=await c.env.DB.prepare('SELECT * FROM users WHERE LOWER(username)=LOWER(?) AND active=1').bind(String(username||'').trim()).first<User>();if(!u||digest(String(password||''))!==u.password_hash)return jsonErr(c,'Invalid username or password',401)
...
app.post('/change-password',async c=>{const a=await auth(c);if(!a||a.kind!=='USER')return jsonErr(c,'Unauthorized',401);const b=await c.req.json();const next=String(b.new_password||'');if(!next)return jsonErr(c,'New password is required');await c.env.DB.prepare('UPDATE users SET password_hash=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(digest(next),a.user!.id).run();return c.json({ok:true})})
...
async function transition(c:any,ref:string,to:string){const a=await auth(c);if(!a)return jsonErr(c,'Unauthorized',401);const b=await bookingByRef(c.env.DB,ref);if(!b)return jsonErr(c,'Booking not found',404);const order:any={PENDING:0,ASSIGNED:1,ACCEPTED:2,CONFIRMED:3,DONE:4};const from=String(b.status||'PENDING');if(a.kind==='ADMIN'){/* controlled admin correction allowed */}else{
 ...
 await recomputeBooking(c.env.DB,ref);if(to==='DONE')await freezeFinancials(c.env.DB,ref);return c.json({ok:true,status:to})
}
...
app.put('/bookings/:reference',async c=>{const a=await auth(c);if(!a)return jsonErr(c,'Unauthorized',401);const ref=String(c.req.param('reference'));const old=await bookingByRef(c.env.DB,ref);if(!old)return jsonErr(c,'Booking not found',404)
 ...
 const b=await c.req.json();const codes=uniqueStrings(Array.isArray(b.tests)?b.tests.map((x:any)=>x.code):[]).slice(0,100);if(!codes.length)return jsonErr(c,'At least one test is required')
...
app.post('/bookings/:reference/extra-tests',async c=>{
 ...
 const body=await c.req.json();const codes=uniqueStrings(Array.isArray(body.tests)?body.tests.map((x:any)=>x.code):[]).slice(0,100);if(!codes.length)return jsonErr(c,'No extra tests selected')
...
app.post('/admin/login',async c=>{const {password}=await c.req.json();await ensureAdminTable(c.env.DB);const row=await c.env.DB.prepare('SELECT password_hash FROM admin_users WHERE id=1').first<any>();const stored=row?.password_hash||c.env.ADMIN_PASSWORD_HASH;if(!stored||digest(String(password||''))!==stored)return jsonErr(c,'Invalid password',401);if(!row?.password_hash&&c.env.ADMIN_PASSWORD_HASH)await c.env.DB.prepare('INSERT OR REPLACE INTO admin_users(id,password_hash) VALUES(1,?)').bind(c.env.ADMIN_PASSWORD_HASH).run();const token=makeToken(c.env.SESSION_SECRET,'a');setCookie(c,'admin_session',token,{httpOnly:true,secure:true,sameSite:'None',path:'/',maxAge:28800});return c.json({ok:true,token})})
app.post('/admin/change-password',async c=>{const a=await auth(c);if(!a||a.kind!=='ADMIN')return jsonErr(c,'Unauthorized',401);const {current_password,new_password}=await c.req.json();await ensureAdminTable(c.env.DB);const row=await c.env.DB.prepare('SELECT password_hash FROM admin_users WHERE id=1').first<any>();const stored=row?.password_hash||c.env.ADMIN_PASSWORD_HASH;if(!stored||digest(String(current_password||''))!==stored)return jsonErr(c,'Current password is incorrect',401);await c.env.DB.prepare('INSERT OR REPLACE INTO admin_users(id,password_hash,updated_at) VALUES(1,?,CURRENT_TIMESTAMP)').bind(digest(String(new_password||''))).run();return c.json({ok:true})})
...
app.delete('/admin/bookings/:reference',async c=>{const a=await auth(c);if(!a||a.kind!=='ADMIN')return jsonErr(c,'Unauthorized',401);const ref=String(c.req.param('reference'));const r=await c.env.DB.prepare('DELETE FROM bookings WHERE reference=?').bind(ref).run();if(!r.meta.changes)return jsonErr(c,'Booking not found',404);return c.json({ok:true,reference:ref})})
...
app.post('/admin/users',async c=>{const a=await auth(c);if(!a||a.kind!=='ADMIN')return jsonErr(c,'Unauthorized',401);const b=await c.req.json();const permissions=uniqueStrings(Array.isArray(b.permissions)?b.permissions:[]).filter(x=>ALL_PERMISSIONS.includes(x));if(!['DOCTOR','REP','SALESMAN'].includes(b.user_type))return jsonErr(c,'Invalid user type');if(!b.username||!b.display_name||!b.password)return jsonErr(c,'Username, display name and password are required');try{const r=await c.env.DB.prepare('INSERT INTO users(username,display_name,password_hash,mobile,user_type,active,permissions_json) VALUES(?,?,?,?,?,?,?)').bind(String(b.username).trim(),String(b.display_name).trim(),digest(String(b.password)),String(b.mobile||''),b.user_type,b.active===false?0:1,JSON.stringify(permissions)).run();return c.json({ok:true,id:r.meta.last_row_id})}catch{return jsonErr(c,'Username already exists',409)}})
app.put('/admin/users/:id',async c=>{const a=await auth(c);if(!a||a.kind!=='ADMIN')return jsonErr(c,'Unauthorized',401);const id=Number(c.req.param('id'));const b=await c.req.json();const permissions=uniqueStrings(Array.isArray(b.permissions)?b.permissions:[]).filter(x=>ALL_PERMISSIONS.includes(x));await c.env.DB.prepare('UPDATE users SET username=?,display_name=?,mobile=?,user_type=?,active=?,permissions_json=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(String(b.username||''),String(b.display_name||''),String(b.mobile||''),b.user_type,b.active?1:0,JSON.stringify(permissions),id).run();if(b.password)await c.env.DB.prepare('UPDATE users SET password_hash=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(digest(String(b.password)),id).run();return c.json({ok:true})})
...
app.get('/admin/financials',async c=>{const a=await auth(c);if(!a||a.kind!=='ADMIN')return jsonErr(c,'Unauthorized',401);const users=(await c.env.DB.prepare(`SELECT id,display_name,user_type FROM users WHERE active=1 ORDER BY display_name`).all()).results||[];const out=[];for(const u of users as any[]){const row=await c.env.DB.prepare(`SELECT COALESCE(SUM(commission_amount + extra_tests_commission_amount),0) AS done_commission FROM bookings WHERE status='DONE' AND commission_owner_user_id=?`).bind(u.id).first<any>();const all=await c.env.DB.prepare(`SELECT COALESCE(SUM(commission_amount),0) AS expected FROM bookings WHERE status!='DONE' AND commission_owner_user_id=?`).bind(u.id).first<any>();const extra=await c.env.DB.prepare(`SELECT COALESCE(SUM(commission_amount),0) AS extra FROM booking_extra_tests WHERE added_by_user_id=?`).bind(u.id).first<any>();const visit=await c.env.DB.prepare(`SELECT COALESCE(SUM(visit_fee_amount),0) AS visit FROM bookings WHERE status='DONE' AND assigned_to_user_id=? AND created_by_type='SALESMAN'`).bind(u.id).first<any>();const paid=await c.env.DB.prepare('SELECT COALESCE(SUM(amount),0) AS paid FROM commission_payments WHERE user_id=?').bind(u.id).first<any>();const visitPaid=await c.env.DB.prepare('SELECT COALESCE(SUM(amount),0) AS paid FROM visit_fee_payments WHERE user_id=?').bind(u.id).first<any>();out.push({...u,pending_confirmed_commission:Number(all?.expected||0),done_commission:Number(row?.done_commission||0),extra_test_commission:Number(extra?.extra||0),visit_fees:Number(visit?.visit||0),commission_paid:Number(paid?.paid||0),visit_fees_paid:Number(visitPaid?.paid||0)})}return c.json({users:out})})
