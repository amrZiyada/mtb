...
export async function api<T=any>(path:string,options:RequestInit={}){
 ...
 if(typeof window!=='undefined'){const token=localStorage.getItem('mtb_admin_token')||localStorage.getItem('mtb_user_token');if(token)headers.set('Authorization',`Bearer ${token}`)}
