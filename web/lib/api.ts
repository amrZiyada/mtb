...
export async function api<T=any>(path:string,options:RequestInit={}){
 ...
 if(typeof window!=='undefined'){const adminRequest=path==='/admin/me'||path.startsWith('/admin/');const token=localStorage.getItem(adminRequest?'mtb_admin_token':'mtb_user_token');if(token)headers.set('Authorization',`Bearer ${token}`)}
