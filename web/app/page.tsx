...
export default function Home(){
  ...
  const [form,setForm]=useState({
    patient_name:'',age:'',gender:'',phone:'',preferred_at:'',address:''
  })

  // Do not load the catalogue on first page load.
  // Only request matching tests after the user starts typing.
  useEffect(()=>{
    const term=q.trim()
    if(!term){
      setTests([])
    ...
    const timer=window.setTimeout(async()=>{
      try{
        setLoading(true)
        setErr('')
        const x=await api<{tests:Test[]}>(`/tests?q=${encodeURIComponent(term)}`)
        if(!controller.signal.aborted){
          setTests(x.tests)
          setCatalog(prev=>{
            const next={...prev}
            for(const t of x.tests) next[t.test_no]=t
            return next
          })
        }
      }catch(e:any){
        if(!controller.signal.aborted) setErr(e.message||'Search failed')
      }finally{
  ...
  const selected=useMemo(
    ()=>Object.keys(cart)
      .filter(code=>cart[code]>0 && catalog[code])
      .map(code=>catalog[code]),
    [cart,catalog]
  )

  const total=useMemo(
    ()=>selected.reduce((sum,t)=>sum+(cart[t.test_no]||0)*Number(t.patient_price),0),
    [selected,cart]
  )

  const addTest=(test:Test)=>{
    setCatalog(prev=>({...prev,[test.test_no]:test}))
    setCart(c=>({...c,[test.test_no]:(c[test.test_no]||0)+1}))
  }
  ...
  const addFirstResult=()=>{
    ...
    addTest(first)
    setQ('')
    // Keep the search input focused so the user can immediately type the next test.
            ...
            {selected.map(t=>(
              <div className="flex items-center justify-between gap-3 text-sm" key={t.test_no}>
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-slate-500">{cart[t.test_no]} × {t.patient_price} EGP</div>
                </div>
                <div className="flex shrink-0 items-center gap-1.5">
                  ...
                  <button aria-label={`Add ${t.analysis_name}`} onClick={()=>addTest(t)} className="h-7 w-7 rounded-lg border">+</button>
                </div>
              </div>
            ))}
          </div>
          <div className="my-3 flex justify-between border-t pt-3 font-bold"><span>Total</span><span>{total} EGP</span></div>
          <button onClick={()=>setOpen(true)} className="w-full rounded-xl bg-slate-900 px-4 py-3 font-semibold text-white">Reserve</button>
        </>
    ...
    <main className="min-h-screen">
      ...
      <div className="mx-auto max-w-6xl px-4 py-4 sm:py-6">
        <div className="lg:hidden">
          <div className="sticky top-0 z-40 -mx-4 border-b bg-slate-50/95 px-4 py-3 backdrop-blur">
            <SearchBox />
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_330px]">
          <section>
            <div className="hidden lg:block lg:sticky lg:top-0 lg:z-20"><SearchBox /></div>
            ...
            <div className="mt-4">
              {!q.trim() ? (
                ...
                <div className="rounded-2xl border bg-white p-8 text-center text-sm text-slate-500">No matching tests found.</div>
              ) : (
                <>
                  <div className="mb-2 text-xs text-slate-500">Showing {tests.length} matching test{tests.length===1?'':'s'}</div>
                      ...
                      <article key={t.test_no} className="rounded-2xl border bg-white p-4 shadow-sm">
                        <div className="flex items-start justify-between gap-4">
                          <div className="min-w-0">
                            ...
                            <div className="mt-1 text-sm text-slate-500">#{t.test_no}{t.unit&&` · ${t.unit}`}{t.specimen&&` · ${t.specimen}`}</div>
                          ...
                          <div className="shrink-0 text-right"><div className="text-lg font-bold">{t.patient_price}</div><div className="text-xs text-slate-500">EGP</div></div>
                        </div>
                        <div className="mt-3 flex items-center gap-2">
                          ...
                          <button aria-label="Add" onClick={()=>addTest(t)} className="h-9 w-9 rounded-lg border">+</button>
                        </div>
                      </article>
                    ))}
                  </div>
            ...
            {/* On mobile, search results stay above the reservation list. */}
            <div className="mt-4 lg:hidden"><CartBox mobile /></div>
          </section>

          <aside className="hidden h-fit lg:sticky lg:top-4 lg:block"><CartBox /></aside>
        </div>
      </div>
      ...
      {open&&(
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4">
          <form onSubmit={submit} className="max-h-[90vh] w-full max-w-xl overflow-auto rounded-2xl bg-white p-6">
            <div className="flex justify-between"><h2 className="text-xl font-bold">Booking details</h2><button type="button" onClick={()=>setOpen(false)}>✕</button></div>
      ...
      {done&&(
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-7 text-center">
            <div className="text-4xl">✓</div><h2 className="mt-3 text-2xl font-bold">Booking received</h2><p className="mt-2 text-slate-600">Reference</p>
